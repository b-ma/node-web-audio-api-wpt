...
globalThis.testAudioNodeOptions = function testAudioNodeOptions(should, context, nodeName, expectedNodeOptions) {
...
globalThis.initializeContext = function initializeContext(should) {
  let c;
  should(() => {
...
globalThis.testInvalidConstructor = function testInvalidConstructor(should, name, context) {
  should(() => {
    new globalThis[name]();
  }, 'new ' + name + '()').throw(TypeError);
  should(() => {
    new globalThis[name](1);
  }, 'new ' + name + '(1)').throw(TypeError);
  should(() => {
    new globalThis[name](context, 42);
  }, 'new ' + name + '(context, 42)').throw(TypeError);
}

globalThis.testDefaultConstructor = function testDefaultConstructor(should, name, context, options) {
  let node;
  ...
  should(() => {
    console.log(new globalThis[name]);
    node = new globalThis[name](context, options.constructorOptions);
  }, message).notThrow();

  console.log(node);

  should(node instanceof globalThis[name], options.prefix + ' instanceof ' + name)
      .beEqualTo(true);
...
globalThis.testDefaultAttributes = function testDefaultAttributes(should, node, prefix, items) {
  items.forEach((item) => {
